...
def delete_post(dcid, gall_url):
    # POST DELETE Headers
    POST_DELETE_REQ_HEADERS = {
    ...
    # Parse Posts
    gallog_posts = sess.get("https://gallog.dcinside.com/" + gall_url)
    posts_parsed = lxml.html.fromstring(gallog_posts.text)

    # Get Values for Service Code
    hidden_r = posts_parsed.xpath('//*[@id="container"]/article/div/section/script[2]')[0].text.strip()
    hidden_r = re.findall("_d\('([\w\0-Z]*)'\)", hidden_r)[0]
    hidden_svc_code = posts_parsed.xpath('//*[@id="container"]/article/div/section/div[1]/input')[0].get("value")

    # Generate Service Code
    svc_code = decodeServiceCode(hidden_svc_code, hidden_r)

    # Get Post Information
    post_gall = posts_parsed.xpath('//*[@id="container"]/article/div/section/div[1]/div/ul/li[1]/div[3]/span/a')[
        0].text
    post_no = posts_parsed.xpath('//*[@id="container"]/article/div/section/div[1]/div/ul/li[1]')[0].get("data-no")
    post_delete_url = "https://gallog.dcinside.com/" + dcid + "/ajax/log_list_ajax/delete"

    POST_DELETE_REQ_DATA = {
    ...
    # POST & GET Result
    delete_result = sess.post(post_delete_url, data=POST_DELETE_REQ_DATA, headers=POST_DELETE_REQ_HEADERS)
    ...
    if result == "captcha":
        ...
        pyperclip.copy("https://gallog.dcinside.com/" + dcid + "/posting")
        print("브라우저 주소창에 Ctrl+V 하시면 바로 갤로그로 이동할 수 있습니다.")
        input("\n엔터키를 입력하면 종료됩니다.")
        exit(1)
    else:
        return f'GallName: {post_gall} / PostNum : {post_no} / Result = {result}'


def get_posts_num(gall_url, flag):
    # Get Gallog
...
def select_gall_list(dcid):
    # Get Gallog
    gallog = sess.get("https://gallog.dcinside.com/" + dcid + "/posting")
...
if __name__ == '__main__':
    print("DCInside Post Cleaner by qwertycvb\n")
    ...
    if selected_gall == dcid + "/posting":
        num = get_posts_num(selected_gall, 0)
    else:
        num = get_posts_num(selected_gall, 1)
    print("총 " + num + "개의 게시글에 대한 삭제를 시도합니다.\n")

    for i in range(int(num)):
        rst = delete_post(dcid, selected_gall)
